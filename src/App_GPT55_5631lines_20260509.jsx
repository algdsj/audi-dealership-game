...
let _nicknameUsed = { dcc: new Set(), sales: new Set(), tech: new Set(), service: new Set(), streamer: new Set() };
...
const getAvatarSeed = (type, member = {}) => Number.isFinite(member.avatarId)
  ? member.avatarId
...
const normalizeStaffMember = (type, member = {}) => {
  ...
  return {
    ...
    traitId: member.traitId || pickTrait(type)?.id,
...
const createStaffMember = (type, skill = 30) => normalizeStaffMember(type, {
  id: `${type}_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`,
  skill,
  retained: false,
  nickname: pickNickname(type),
  traitId: pickTrait(type)?.id,
});
...
const EMPTY_LEAD_CHANNELS = LEAD_CHANNELS.reduce((acc, channel) => ({ ...acc, [channel.id]: 0 }), {});
const sumLeadChannels = (channels = {}) => Object.values({ ...EMPTY_LEAD_CHANNELS, ...channels }).reduce((sum, value) => sum + (Number(value) || 0), 0);
const normalizeLeadChannels = (marketingState = {}) => {
...
const CUSTOMER_NAMES = ['陈先生', '林女士', '周总', '王先生', '赵女士', '刘先生', '张经理', '许女士', '何先生', '马女士'];
...
const STAFF_TRAITS = {
  ...
  service: [
    { id: 'patient', name: '耐心型', desc: '更擅长安抚客户，CSI恢复更快。', csiBonus: 0.18, complaintMult: 0.88 },
    { id: 'callback', name: '回访型', desc: '能带动更多客户回厂保养。', returnVisitBonus: 0.12 },
    { id: 'process', name: '流程型', desc: '投诉闭环稳定，但成长稍慢。', complaintMult: 0.82, trainBonus: -1 },
  ],
  streamer: [
    { id: 'traffic', name: '流量型', desc: '直播线索产出更高。', leadBonus: 0.16 },
    { id: 'product', name: '产品型', desc: '讲车专业，直播线索到店率更高。', livestreamWalkinBonus: 0.08 },
    { id: 'volatile', name: '情绪型', desc: '爆发强但稳定性差。', leadBonus: 0.24, turnoverMult: 1.25 },
  ],
};
...
const normalizeFeedbackState = (value = {}) => ({
  ...DEFAULT_FEEDBACK,
  ...value,
  unlockedAchievementIds: Array.isArray(value.unlockedAchievementIds) ? value.unlockedAchievementIds : [],
  monthlyBadges: Array.isArray(value.monthlyBadges) ? value.monthlyBadges : [],
...
const buildLossDrivers = (stats, currentNetProfit) => {
...
const getEffectiveSkill = (type, member) => Math.max(1, Math.min(100, (member?.skill || 0) + (getTrait(type, member)?.skillBonus || 0) + (getCareerLevel(member)?.skillBonus || 0)));
const estimateTurnoverRiskPercent = (type, member, regionTurnover = 1) => {
  ...
  risk *= getTrait(type, normalized)?.turnoverMult || 1;
...
const StaffAvatar = ({ type, member, size = 44 }) => {
...
export default function App() {
  ...
  const [marketing, setMarketing] = useState({
    budget: 2000,       // 旧版兼容：等同于线索采买日预算
    leadPurchaseBudget: 2000, // 线索采买日预算
    livestreamBudget: 0,      // 直播投流日预算
    leads: 0,           // 待处理线索池总数（兼容旧存档）
    leadChannels: { ...EMPTY_LEAD_CHANNELS }, // 分渠道线索池
  ...
  const serviceAvgSkill = serviceCount > 0 ? Math.round(staff.service.members.reduce((s, m) => s + getEffectiveSkill('service', m), 0) / serviceCount) : 0;
  const streamerAvgSkill = streamerCount > 0 ? Math.round(staff.streamer.members.reduce((s, m) => s + getEffectiveSkill('streamer', m), 0) / streamerCount) : 0;
  ...
  const [monthlyStats, setMonthlyStats] = useState({
    ...
    afterSalesReturnVisits: 0,      // 本月回厂台次
  ...
  const createCustomerDealCase = ({ channelId = 'showroom', segment = null, sourceDay = day, stockList = inventory }) => {
  ...
  const resolveNegotiation = (caseId, optionId) => {
    ...
    if (item.kind === 'manufacturer_subsidy') {
      ...
      setMarketing(prev => {
        const channels = normalizeLeadChannels(prev);
        channels[channel] += leadBonus;
        return { ...prev, leadChannels: channels, leads: sumLeadChannels(channels) };
      });
      setMonthlyStats(prev => ({ ...prev, leads: prev.leads + leadBonus, referralLeads: prev.referralLeads + (option.id === 'tradein' ? 15 : 0) }));
      addLog('success', `📣【厂家营销支持】获得 ${leadBonus} 条${LEAD_CHANNELS.find(c => c.id === channel)?.name || ''}线索${option.id === 'tradein' ? '，其中一批为置换意向客户' : ''}。`);
  ...
  const handleNextDay = () => {
    ...
    m.leadPurchaseBudget = Number.isFinite(m.leadPurchaseBudget) ? m.leadPurchaseBudget : (m.budget || 0);
    m.livestreamBudget = Number.isFinite(m.livestreamBudget) ? m.livestreamBudget : 0;
    m.budget = m.leadPurchaseBudget;
    m.leads = sumLeadChannels(m.leadChannels);
    let currentLogs = [];
    let inboxItems = [];
    ...
    nextCustomerDeals = nextCustomerDeals.filter(item => item.dueDay >= day + 1);
    ...
    for (const item of expiredCustomerDeals) {
      currentLogs.push({ day: day + 1, type: 'warning', message: `👤【重点客户流失】${item.customerName}（${item.archetypeName}）没有及时谈判，转向竞品门店。` });
    }
    ...
    const serviceRetainedCount = (staff.service?.members || []).filter(m => m.retained).length;
    const streamerRetainedCount = (staff.streamer?.members || []).filter(m => m.retained).length;
    const retentionBonus = (dccRetainedCount * staff.dcc.salary) + (salesRetainedCount * staff.sales.salary) + (serviceRetainedCount * (staff.service?.salary || STAFF_ROLE_META.service.salary)) + (streamerRetainedCount * (staff.streamer?.salary || STAFF_ROLE_META.streamer.salary));
    const techRetainedCount = afterSales.technicians.filter(m => m.retained).length;
    const salaries = (dccCount * staff.dcc.salary) + (salesCount * staff.sales.salary) + (serviceCount * (staff.service?.salary || STAFF_ROLE_META.service.salary)) + (streamerCount * (staff.streamer?.salary || STAFF_ROLE_META.streamer.salary)) + retentionBonus + (techCount * afterSales.salary) + (techRetainedCount * afterSales.salary);
    ...
    const leadPurchaseBudget = Math.max(0, m.leadPurchaseBudget || 0);
    const livestreamBudget = Math.max(0, m.livestreamBudget || 0);
    ...
    mStats.marketingCost += dailyMarketingBudget;
    ...
    todayLedger.push({ label: `人工薪酬(${dccCount + salesCount + serviceCount + streamerCount + techCount}人)`, amount: -salaries, type: 'expense' });
    if (leadPurchaseBudget > 0) todayLedger.push({ label: '线索采买', amount: -leadPurchaseBudget, type: 'expense' });
    if (livestreamBudget > 0) todayLedger.push({ label: '直播投流', amount: -livestreamBudget, type: 'expense' });
    ...
    const calcTurnoverRisk = (type, member) => {
      return estimateTurnoverRiskPercent(type, member, activeRegion.turnover || 1) / 100;
    };
    ...
    checkStaffTurnover('service');
    checkStaffTurnover('streamer');
    checkTechTurnover();

    // 处理离职
    let updatedDccMembers = [...staff.dcc.members];
    let updatedSalesMembers = [...staff.sales.members];
    let updatedServiceMembers = [...(staff.service?.members || [])];
    let updatedStreamerMembers = [...(staff.streamer?.members || [])];
    ...
    if (lostMembers.length > 0) {
      for (const { type, member } of lostMembers) {
        ...
        else if (type === 'service') updatedServiceMembers = updatedServiceMembers.filter(m => m.id !== member.id);
        else if (type === 'streamer') updatedStreamerMembers = updatedStreamerMembers.filter(m => m.id !== member.id);
    ...
    const streamerEfficiency = streamerCount > 0 ? Math.min(2.2, 0.45 + streamerCount * 0.18 + streamerAvgSkill / 120 + traitSum('streamer', staff.streamer?.members || [], 'leadBonus')) : 0;
    const livestreamLeads = Math.floor((livestreamBudget / (70 * (activeRegion.leadCost || 1))) * (0.65 + Math.random() * 0.7) * (activeRegion.demand || 1) * streamerEfficiency);
    if (newLeads > 0) m.leadChannels.sourcing += newLeads;
    if (livestreamLeads > 0) m.leadChannels.livestream += livestreamLeads;
    m.leads = sumLeadChannels(m.leadChannels);
    stats.newLeads = newLeads + livestreamLeads;
    mStats.leads += newLeads + livestreamLeads; 
    if (livestreamLeads > 0) {
      currentLogs.push({ day: day + 1, type: 'success', message: `🎙️【直播投流】主播团队承接投流，带来 ${livestreamLeads} 条直播线索。` });
    }
    if (livestreamBudget > 0 && streamerCount === 0) {
      currentLogs.push({ day: day + 1, type: 'warning', message: `🎙️【直播投流浪费】今天投入了直播预算，但没有主播承接，未产生直播线索。` });
    }
    ...
    if (currentLeadTotal > 0) {
      ...
      LEAD_CHANNELS.forEach((channel, index) => {
        ...
        const livestreamBonus = channel.id === 'livestream' ? Math.min(0.12, streamerAvgSkill / 1000 + traitSum('streamer', staff.streamer?.members || [], 'livestreamWalkinBonus')) : 0;
    ...
    m.leadChannels = remainingLeadChannels;
    m.leads = sumLeadChannels(m.leadChannels);
    ...
    const processedSummary = LEAD_CHANNELS
      ...
      .join(' / ');
    if (processedSummary) currentLogs.push({ day: day + 1, type: 'info', message: `📞【线索跟进】DCC处理 ${processedLeads} 条线索（${processedSummary}），邀约到店 ${dccWalkIns} 批。` });
    ...
    if (updatedInventory.length > 0 && handledCustomers.length > 0 && nextCustomerDeals.length < 6) {
      const channelCandidates = LEAD_CHANNELS.flatMap(channel => Array.from({ length: Math.max(0, processedByChannel[channel.id] || 0) }, () => channel.id));
      ...
      let generatedCases = [];
      let reservedCarIds = new Set(nextCustomerDeals.map(item => item.carId));
      for (let i = 0; i < maxNewCases; i++) {
        if (Math.random() > 0.75 && generatedCases.length > 0) continue;
        const stockForCase = updatedInventory.filter(car => !reservedCarIds.has(car.id));
        if (stockForCase.length === 0) break;
        const channelId = channelCandidates.length > 0
          ? channelCandidates[Math.floor(Math.random() * channelCandidates.length)]
          : 'showroom';
        const segment = handledCustomers[Math.floor(Math.random() * handledCustomers.length)];
        const dealCase = createCustomerDealCase({ channelId, segment, sourceDay: day + 1, stockList: stockForCase });
        if (dealCase) {
      ...
      if (generatedCases.length > 0) {
        ...
        currentLogs.push({ day: day + 1, type: 'info', message: `👤【重点客户】销售团队筛出 ${generatedCases.length} 位可谈判客户，请在“客户谈判”中及时处理。` });
      }
    }
    ...
    for (let i = 0; i < updatedSoldVehicles.length; i++) {
      ...
      if (Math.random() < (1/30) * (1 + serviceReturnBoost)) {
    ...
    mStats.afterSalesReturnVisits = (mStats.afterSalesReturnVisits || 0) + asOrdersHandled;
    if (dailyAsRevenue > 0) {
      ...
      if (serviceReturnBoost > 0) todayLedger.push({ label: `客服回厂加成(${Math.round(serviceReturnBoost * 100)}%)`, amount: 0, type: 'pending' });
    }
    ...
    if (newCsiScore >= 95) {
      ...
      m.leadChannels.showroom += referralCount;
      m.leads = sumLeadChannels(m.leadChannels);
    ...
    const advanceCareer = (members, type, profile) => {
      const roleName = STAFF_ROLE_META[type]?.label || '员工';
      return members.map(member => {
        ...
        if (afterLevel.level > beforeLevel.level) {
          currentLogs.push({ day: day + 1, type: 'success', message: `⭐【员工成长】${roleName}【${next.nickname}】晋升为${afterLevel.title}，有效能力获得成长加成。` });
        } else if (next.stress >= 82 && before.stress < 82) {
          currentLogs.push({ day: day + 1, type: 'warning', message: `😓【员工压力】${roleName}【${next.nickname}】压力已到${Math.round(next.stress)}，离职风险上升，建议培训、留任或补人分流。` });
        } else if (next.loyalty <= 35 && before.loyalty > 35) {
          currentLogs.push({ day: day + 1, type: 'warning', message: `📉【忠诚预警】${roleName}【${next.nickname}】忠诚度跌至${Math.round(next.loyalty)}，团队文化开始松动。` });
        }
        return next;
      });
    };
    updatedDccMembers = advanceCareer(updatedDccMembers, 'dcc', {
    ...
    updatedSalesMembers = advanceCareer(updatedSalesMembers, 'sales', {
      xp: 1 + handledCustomers.length / Math.max(1, salesCount) * 0.9 + stats.sales / Math.max(1, salesCount) * 5,
      stress: handledCustomers.length >= salesCapacity && salesCapacity > 0 ? 5 : handledCustomers.length > 0 ? 2 : -1,
      loyalty: stats.sales > 0 ? 0.45 : -0.15,
    });
    updatedServiceMembers = advanceCareer(updatedServiceMembers, 'service', {
    ...
    updatedStreamerMembers = advanceCareer(updatedStreamerMembers, 'streamer', {
    ...
    updatedTechMembers = advanceCareer(updatedTechMembers, 'tech', {
    ...
    if (dayOfMonth === 30) {
      ...
      monthlyFeedbackReport = buildMonthlyFeedbackReport({
        monthNo: month,
        monthStats: mStats,
        finalPayout,
        investorReview,
    ...
    setStaff(s => ({
      ...
      service: { ...(s.service || { salary: STAFF_ROLE_META.service.salary }), members: updatedServiceMembers },
      streamer: { ...(s.streamer || { salary: STAFF_ROLE_META.streamer.salary }), members: updatedStreamerMembers },
    }));
    ...
    try {
      ...
      const newStaffObj = {
        ...staff,
        dcc: { ...staff.dcc, members: updatedDccMembers },
        sales: { ...staff.sales, members: updatedSalesMembers },
        service: { ...(staff.service || { salary: STAFF_ROLE_META.service.salary }), members: updatedServiceMembers },
        streamer: { ...(staff.streamer || { salary: STAFF_ROLE_META.streamer.salary }), members: updatedStreamerMembers },
      };
      const autoSaveData = {
        version: 1,
  ...
  const handleGenerateAIAd = async () => {
    ...
    showConfirm("确认启动 AI 营销", `是否支付 ¥${cost.toLocaleString()} 预算，由 AI 店总生成一次专属营销活动？`, async () => {
      ...
      setMarketing(m => {
        const channels = normalizeLeadChannels(m);
        channels.livestream += bonusLeads;
        return { ...m, leadChannels: channels, leads: sumLeadChannels(channels) };
      });
  ...
  const handleCustomerDeal = (caseId, mode) => {
    ...
    const isTradeIn = Math.random() < item.tradeInIntent;
    ...
    setFinance(prev => {
      let nextCash = prev.cash + cashIn;
      let nextLoan = Math.max(0, prev.loan - autoLoanRepay);
      if (nextCash < 0) {
        nextLoan += Math.abs(nextCash);
        nextCash = 0;
      }
      return { ...prev, cash: nextCash, loan: nextLoan };
    });
    if (newUsedCar) setUsedCars(prev => [...prev, newUsedCar]);
    setMonthlyStats(prev => ({
      ...prev,
      sales: prev.sales + 1,
      revenue: prev.revenue + finalPrice,
      cogs: prev.cogs + modelDef.baseCost,
      baseRebatesPool: prev.baseRebatesPool + addons.rebate + tradeInSubsidy,
      derivativeRevenue: prev.derivativeRevenue + addons.derivativeProfit,
      financeCommission: prev.financeCommission + financeCommission,
      tradeInCount: prev.tradeInCount + (isTradeIn ? 1 : 0),
      tradeInSubsidy: prev.tradeInSubsidy + tradeInSubsidy,
    }));
    ...
    setCsi(prev => ({ ...prev, score: Math.max(50, Math.min(100, prev.score + config.csiAdj)) }));
    appendLedger([
  ...
  const handleTrainTech = (techId) => {
    ...
    showConfirm("培训技师", `支付 ¥${cost.toLocaleString()} 培训 ${member.nickname}？`, () => {
      ...
      setAfterSales(prev => ({
        ...prev,
        technicians: prev.technicians.map(m => m.id === techId ? normalizeStaffMember('tech', { ...m, skill: Math.min(100, m.skill + trainGain), xp: (m.xp || 0) + 25, loyalty: (m.loyalty ?? 62) + 2, stress: (m.stress ?? 18) - 4 }) : m)
      }));
      addLog('info', `🔧【技师培训】${member.nickname} 培训完成，能力+${trainGain}，忠诚提升，压力下降。`);
    });
  };
  const toggleTechRetention = (techId) => {
    setAfterSales(prev => ({
      ...prev,
      technicians: prev.technicians.map(m => m.id === techId ? normalizeStaffMember('tech', {
        ...m,
        retained: !m.retained,
        loyalty: (m.loyalty ?? 62) + (!m.retained ? 8 : -2),
        stress: (m.stress ?? 18) + (!m.retained ? -3 : 0),
      }) : m)
    }));
  };
  ...
  const trainMember = (type, memberId) => {
    ...
    showConfirm("确认培训", `确定花费 ¥${cost.toLocaleString()} 为 ${roleName}【${member.nickname}】组织专项培训吗？\n能力值将从 ${member.skill} 提升至 ${Math.min(100, member.skill + 10)}。`, () => {
      ...
      setStaff(s => ({
        ...
        [type]: {
          ...s[type],
          members: s[type].members.map(m => m.id === memberId ? normalizeStaffMember(type, { ...m, skill: newSkill, xp: (m.xp || 0) + 28, loyalty: (m.loyalty ?? 62) + 3, stress: (m.stress ?? 18) - 5 }) : m)
        }
      }));
      addLog('success', `花费 ${formatMoney(cost)} 为${roleName}【${member.nickname}】组织专项培训，能力值 ${member.skill} → ${newSkill}，忠诚提升，压力下降。`);
    });
  };
  ...
  const hireStaff = (type) => {
    ...
    showConfirm("确认招聘", `确定招聘一名新的 ${roleName} 吗？将增加每日固定的工资支出。`, () => {
      const newMember = createStaffMember(type, 20 + Math.floor(Math.random() * 20));
  ...
  const launchActivity = (activityId) => {
    ...
    showConfirm(`确认发起${act.name}`, `是否花费 ¥${act.cost.toLocaleString()} 发起【${act.name}】？\n${act.description}`, () => {
      ...
      if (act.leadBonus) {
        setMarketing(m => {
          const channels = normalizeLeadChannels(m);
          channels[act.channel || 'showroom'] += act.leadBonus;
          return { ...m, leadChannels: channels, leads: sumLeadChannels(channels) };
        });
        setMonthlyStats(prev => ({ ...prev, leads: prev.leads + act.leadBonus }));
      }
      ...
      if (act.effect === 'recover') {
        ...
        setMarketing(m => {
          const channels = normalizeLeadChannels(m);
          channels[act.channel || 'showroom'] += recovered;
          return { ...m, leadChannels: channels, leads: sumLeadChannels(channels) };
        });
        setMonthlyStats(prev => ({ ...prev, recoveredLeads: (prev.recoveredLeads || 0) + recovered, leads: prev.leads + recovered }));
        addLog('success', `🤝【老客维系】DCC回访战败客户，成功回收 ${recovered} 条展厅线索进入跟进池！`);
      } else {
        addLog('success', `${act.icon}【${act.name}】活动已启动！${act.description}${act.leadBonus ? `，新增${act.leadBonus}条${LEAD_CHANNELS.find(c => c.id === act.channel)?.name || ''}线索` : ''}，持续${act.duration}天。`);
      }
    });
  };
  ...
  const toggleRetention = (type, memberId) => {
    ...
    if (!member.retained) {
      ...
      showConfirm("确认留任", `为${roleName}【${member.nickname}】（能力值 ${member.skill}）发放留任津贴？\n每日额外支出 ¥${staff[type].salary}（即月薪翻倍），可有效降低其离职风险。`, () => {
        setStaff(s => ({
          ...s,
          [type]: { ...s[type], members: s[type].members.map(m => m.id === memberId ? normalizeStaffMember(type, { ...m, retained: true, loyalty: (m.loyalty ?? 62) + 8, stress: (m.stress ?? 18) - 3 }) : m) }
        }));
      });
    } else {
      setStaff(s => ({
        ...s,
        [type]: { ...s[type], members: s[type].members.map(m => m.id === memberId ? normalizeStaffMember(type, { ...m, retained: false, loyalty: (m.loyalty ?? 62) - 2 }) : m) }
      }));
    }
  };
  ...
  const applySaveData = (saveData) => {
    ...
    const rawSavedMarketing = saveData.marketing || {};
    const savedMarketing = { budget: 2000, leadPurchaseBudget: rawSavedMarketing.leadPurchaseBudget ?? rawSavedMarketing.budget ?? 2000, livestreamBudget: rawSavedMarketing.livestreamBudget ?? 0, leads: 0, activeActivities: [], ...rawSavedMarketing };
    savedMarketing.leadPurchaseBudget = savedMarketing.leadPurchaseBudget ?? savedMarketing.budget ?? 2000;
    savedMarketing.livestreamBudget = savedMarketing.livestreamBudget ?? 0;
    savedMarketing.budget = savedMarketing.leadPurchaseBudget;
    const savedLeadChannels = normalizeLeadChannels(savedMarketing);
    setMarketing({ ...savedMarketing, leadChannels: savedLeadChannels, leads: sumLeadChannels(savedLeadChannels) });
    const savedStaff = saveData.staff || { dcc: { members: [], salary: 150 }, sales: { members: [], salary: 250 } };
    setStaff({
      dcc: { salary: STAFF_ROLE_META.dcc.salary, ...savedStaff.dcc, members: (savedStaff.dcc?.members || []).map(m => normalizeStaffMember('dcc', m)) },
      sales: { salary: STAFF_ROLE_META.sales.salary, ...savedStaff.sales, members: (savedStaff.sales?.members || []).map(m => normalizeStaffMember('sales', m)) },
      service: { salary: STAFF_ROLE_META.service.salary, ...(savedStaff.service || {}), members: (savedStaff.service?.members || [createStaffMember('service', 30)]).map(m => normalizeStaffMember('service', m)) },
      streamer: { salary: STAFF_ROLE_META.streamer.salary, ...(savedStaff.streamer || {}), members: (savedStaff.streamer?.members || []).map(m => normalizeStaffMember('streamer', m)) },
    });
    const savedAfterSales = saveData.afterSales || { technicians: [], salary: 200 };
    setAfterSales({ ...savedAfterSales, technicians: (savedAfterSales.technicians || []).map(m => normalizeStaffMember('tech', m)) });
    ...
    setCustomerDeals(saveData.customerDeals || []);
    setFeedback(normalizeFeedbackState(saveData.feedback));
  ...
  const operatingScore = Math.max(0, Math.min(100, Math.round(
    salesPaceScore +
  ...
  const careerMembers = [
    ...staff.dcc.members.map(member => ({ type: 'dcc', member })),
    ...staff.sales.members.map(member => ({ type: 'sales', member })),
    ...(staff.service?.members || []).map(member => ({ type: 'service', member })),
    ...(staff.streamer?.members || []).map(member => ({ type: 'streamer', member })),
    ...afterSales.technicians.map(member => ({ type: 'tech', member })),
  ];
  ...
  const dailyChecklist = [
    ...
    {
      title: '展厅保持可卖',
      done: inventory.length > 0 && (showroomUsedForTasks > 0 || pendingOrders.length > 0),
      detail: inventory.length > 0 ? `当前展车${showroomUsedForTasks}/${facility.showroomSpots}。` : '没有现车会让客流和谈判断档。',
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
                    {MARKETING_ACTIVITIES.map(act => {
                      ...
                      const dailyBudget = act.costType === 'daily'
                        ? (marketing[act.budgetKey] ?? (act.budgetKey === 'leadPurchaseBudget' ? marketing.budget : 0) ?? 0)
                        : act.cost;
                      return (
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  setMarketing(m => ({
                                    ...m,
                                    [act.budgetKey]: v,
                                    ...(act.budgetKey === 'leadPurchaseBudget' ? { budget: v } : {}),
                                  }));
                                }}
                ...
                <div className="animate-fade-in">
                  ...
                  {customerDeals.filter(item => item.status === 'pending').length === 0 ? (
                    ...
                    <div className="space-y-4">
                      {customerDeals.filter(item => item.status === 'pending').map(item => {
                        ...
                        const financeProfit = estimateDealAddons(item.modelId, item.targetPrice).grossProfit + Math.round(estimateDealAddons(item.modelId, item.targetPrice).financeCommission * 0.45);
                        return (
                          <div key={item.id} className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                            ...
                            <div className="p-5 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
                              <button onClick={() => handleCustomerDeal(item.id, 'margin')} className="text-left rounded-lg border border-slate-200 hover:border-slate-400 hover:bg-slate-50 p-3 transition-colors">
                              ...
                              <button onClick={() => handleCustomerDeal(item.id, 'balanced')} className="text-left rounded-lg border border-blue-200 hover:border-blue-400 bg-blue-50/40 hover:bg-blue-50 p-3 transition-colors">
                              ...
                              <button onClick={() => handleCustomerDeal(item.id, 'close')} className="text-left rounded-lg border border-red-200 hover:border-red-400 bg-red-50/40 hover:bg-red-50 p-3 transition-colors">
                            ...
                            <div className="px-5 pb-5">
                              <button onClick={() => handleCustomerDeal(item.id, 'reject')} className="text-xs font-bold text-slate-500 hover:text-red-600 transition-colors">
                                放弃该客户
                              </button>
                            </div>
                          </div>
              ...
              {activeTab === 'staff' && (
                <div className="animate-fade-in">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="border border-slate-200 rounded-xl p-5">
                      ...
                      <div className="space-y-2 max-h-64 overflow-y-auto">
                        ...
                        {staff.dcc.members.map(member => {
                          ...
                          const risk = estimateTurnoverRiskPercent('dcc', member, activeRegion.turnover || 1);
                          ...
                          return (
                            <div key={member.id} className={'p-3 rounded-lg border ' + (member.retained ? 'border-blue-200 bg-blue-50/50' : 'border-slate-100 bg-white')}>
                              <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-2">
                                  <StaffAvatar type="dcc" member={member} />
                                  <div>
                    ...
                    {['service', 'streamer'].map(type => {
                      const team = staff[type] || { members: [], salary: STAFF_ROLE_META[type].salary };
                      const avgSkill = type === 'service' ? serviceAvgSkill : streamerAvgSkill;
                      const headline = type === 'service'
                        ? '提升CSI自然恢复，降低投诉概率，并增加售后回厂台次。'
                        : '配合直播投流，把流量预算转成直播线索。';
                      return (
                        <div key={type} className="border border-slate-200 rounded-xl p-5">
                          <div className="flex justify-between items-center mb-3">
                            <div>
                              ...
                              <p className="text-[10px] text-slate-400 mt-0.5">{headline}</p>
                            </div>
                            <button onClick={() => hireStaff(type)} className="px-3 py-1.5 border border-slate-300 rounded text-xs font-bold hover:bg-slate-50">+招聘</button>
                          </div>
                          <div className="grid grid-cols-2 gap-3 mb-3">
                            <div className="bg-slate-50 p-2 rounded text-center"><p className="text-[10px] text-slate-400">人数/日薪</p><p className="font-bold text-sm">{team.members.length} / ¥{team.salary}</p></div>
                          ...
                          <div className="space-y-2 max-h-64 overflow-y-auto">
                            {team.members.length === 0 && <p className="text-xs text-slate-400 text-center py-4">{type === 'streamer' ? '暂无主播，招聘后直播投流无法转化为线索' : '团队已空，请招聘新人'}</p>}
                            {team.members.map(member => {
                              const trait = getTrait(type, member);
                                ...
                                <div key={member.id} className={'p-3 rounded-lg border ' + (member.retained ? 'border-blue-200 bg-blue-50/50' : 'border-slate-100 bg-white')}>
                                  <div className="flex items-center justify-between mb-2">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <div>
                                        <p className="text-sm font-bold text-slate-800">{member.nickname}</p>
                                        <p className="text-[10px] text-slate-400">{STAFF_ROLE_META[type].label} · 日薪 ¥{team.salary}{member.retained ? ' +留任¥' + team.salary : ''}</p>
                                        {trait && <p className="text-[10px] text-indigo-600 font-bold mt-0.5">{trait.name} · {trait.desc}</p>}
                                        <p className="text-[10px] text-slate-400 mt-0.5">有效能力 {effectiveSkill} · 等级加成 +{getCareerLevel(member).skillBonus}</p>
                                      </div>
                                    </div>
                                    <div className="flex items-center gap-1.5">
                                      <span className={'text-[10px] font-bold px-1.5 py-0.5 rounded border ' + riskColor}>流失{effectiveRisk}%</span>
                                      <button
                                        onClick={() => toggleRetention(type, member.id)}
                                        className={`text-[10px] font-bold px-2 py-1 rounded transition-colors ${
                                          member.retained
                                            ? 'bg-blue-100 text-blue-700 border border-blue-300'
                                            : risk >= 2 ? 'bg-red-50 text-red-600 border border-red-200 hover:bg-red-100' : 'bg-slate-50 text-slate-400 border border-slate-200'
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] font-bold text-slate-500 w-7 text-right">{member.skill}</span>
                                    <button
                                      onClick={() => trainMember(type, member.id)}
                                      disabled={member.skill >= 100}
                                      className="text-[10px] font-bold px-2 py-1 rounded bg-indigo-50 text-indigo-700 border border-indigo-200 hover:bg-indigo-100 disabled:opacity-40 disabled:cursor-not-allowed transition-colors whitespace-nowrap"
                                    >
                                      培训¥2万
                    ...
                    <div className="border border-slate-200 rounded-xl p-5 lg:col-span-2">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
                        ...
                        <div className="bg-slate-50 p-2 rounded text-center"><p className="text-[10px] text-slate-400">留任人数</p><p className="font-bold text-sm text-blue-600">{staff.sales.members.filter(m => m.retained).length + staff.dcc.members.filter(m => m.retained).length + (staff.service?.members || []).filter(m => m.retained).length + (staff.streamer?.members || []).filter(m => m.retained).length} 人</p></div>
                        <div className="bg-slate-50 p-2 rounded text-center"><p className="text-[10px] text-slate-400">留任日支出</p><p className="font-bold text-sm text-orange-600">{formatMoney((staff.sales.members.filter(m => m.retained).length * staff.sales.salary) + (staff.dcc.members.filter(m => m.retained).length * staff.dcc.salary) + ((staff.service?.members || []).filter(m => m.retained).length * (staff.service?.salary || STAFF_ROLE_META.service.salary)) + ((staff.streamer?.members || []).filter(m => m.retained).length * (staff.streamer?.salary || STAFF_ROLE_META.streamer.salary)))}</p></div>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                        ...
                        {staff.sales.members.map(member => {
                          ...
                          const risk = estimateTurnoverRiskPercent('sales', member, activeRegion.turnover || 1);
                          const effectiveRisk = Number(risk.toFixed(1));
                          const riskLevel = risk >= 4 ? 'high' : risk >= 2 ? 'mid' : 'low';
                          const riskColor = riskLevel === 'high' ? 'text-red-600 bg-red-50 border-red-200' : riskLevel === 'mid' ? 'text-amber-600 bg-amber-50 border-amber-200' : 'text-green-600 bg-green-50 border-green-200';
                          return (
                            <div key={member.id} className={'p-3 rounded-lg border ' + (member.retained ? 'border-blue-200 bg-blue-50/50' : 'border-slate-100 bg-white')}>
                              <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-2">
                                  <StaffAvatar type="sales" member={member} />
                                  <div>
                                    <p className="text-sm font-bold text-slate-800">{member.nickname}</p>
                    ...
                    <div className="border border-slate-200 rounded-xl p-5 lg:col-span-2">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                        ...
                        {afterSales.technicians.map(member => {
                          ...
                          const risk = estimateTurnoverRiskPercent('tech', member, activeRegion.turnover || 1);
                          const effectiveRisk = Number(risk.toFixed(1));
                          const riskColor = risk >= 4 ? 'text-red-600 bg-red-50 border-red-200' : risk >= 2 ? 'text-amber-600 bg-amber-50 border-amber-200' : 'text-green-600 bg-green-50 border-green-200';
                          return (
                            <div key={member.id} className={'p-3 rounded-lg border ' + (member.retained ? 'border-blue-200 bg-blue-50/50' : 'border-slate-100 bg-white')}>
                              <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-2">
                                  <StaffAvatar type="tech" member={member} />
                                  <div>
                                    <p className="text-sm font-bold text-slate-800">{member.nickname}</p>
